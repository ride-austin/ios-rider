...
- (void)configure {
    [super configure];
    UIEdgeInsets insets = UIEdgeInsetsMake(11, 13, 11, 13);
    self.textView.delegate = self;
    self.textView.textContainerInset = insets;
    self.placeholderView.textContainerInset = insets;
...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text {
    ...
    if (![text isEqualToString:@""] && ![self.textFieldModel.validCharacters containsString:text]) {
        return NO;
    }
