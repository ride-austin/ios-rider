...
- (void)textViewDidBeginEditing:(UITextView *)textView {
    if ([self.tvIssueDetails.text isEqualToString:kPlaceHolderText] && !self.validDetails) {
...
- (void)textViewDidEndEditing:(UITextView *)textView {
    if ([self.tvIssueDetails.text isEqualToString:@""]) {
        self.tvIssueDetails.text = kPlaceHolderText;
        self.tvIssueDetails.textColor = kPlaceHolderColor;
        self.validDetails = NO;
        return;
    }
...
- (IBAction)submitPressed:(id)sender {
    ...
    [SupportTopicAPI postSupportMessage:supportMessage supportTopic:self.selectedSupportTopic rideId:self.tripHistoryDataModel.rideId withCompletion:^(NSError *error) {
        [self hideHUD];
        if (!error) {
